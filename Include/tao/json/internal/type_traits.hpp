...
namespace tao::json::internal
{
    ...
    {
        [[nodiscard]] bool operator()(const std::type_info *l, const std::type_info *r) const { return l->before(*r); }
    };

    template <typename...> inline constexpr bool has_as_impl = false;
    ...
    inline constexpr bool has_as_type_impl<
        decltype((void)Traits<T>::template as_type<Traits, T>(std::declval<Args>()...)), Traits, T, Args...> = true;
    ...
    template <typename...> inline constexpr bool has_to_impl = false;
    ...
    inline constexpr bool
        has_consume_one<Traits, P, U, decltype((void)Traits<U>::template consume<Traits>(std::declval<P &>()))> = true;

    template <template <typename...> class, typename, typename, typename = void>
    inline constexpr bool has_consume_two = false;

    template <template <typename...> class Traits, typename P, typename U>
    inline constexpr bool has_consume_two<
        Traits, P, U, decltype((void)Traits<U>::template consume<Traits>(std::declval<P &>(), std::declval<U &>()))> =
        true;
    ...
    inline constexpr bool
        has_is_nothing<Traits, T, decltype((void)Traits<T>::template is_nothing<Traits>(std::declval<const T &>()))> =
            true;

    template <template <typename...> class Traits, typename T>
    [[nodiscard]] std::enable_if_t<has_is_nothing<Traits, T>, bool> is_nothing(const T &t) noexcept(
        noexcept(Traits<T>::template is_nothing<Traits>(t)))
    {
    ...
    template <template <typename...> class Traits, typename T>
    inline constexpr bool
        has_enable_implicit_constructor<Traits, T, decltype((void)Traits<T>::enable_implicit_constructor)> = true;

    template <template <typename...> class Traits, typename T>
    inline constexpr bool enable_implicit_constructor =
        Traits<std::conditional_t<has_enable_implicit_constructor<Traits, T>, T, void>>::enable_implicit_constructor;
